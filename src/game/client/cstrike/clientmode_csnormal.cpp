...
void ClientModeCSNormal::Shutdown()
{
...
void ClientModeCSNormal::FireGameEvent( IGameEvent *event )
{
	...
	else if ( Q_strcmp( "bomb_planted", eventname ) == 0 )
	{
		...
		// show centerprint message
		GetCenterPrint()->Print( "#Cstrike_TitlesTXT_Bomb_Planted" );
	...
	else if ( Q_strcmp( "player_death", eventname ) == 0 )
	{
		...
		if ( pPlayer == C_BasePlayer::GetLocalPlayer() )
		{
			...
			GetViewPortInterface()->ShowPanel( PANEL_BUY_CT, false );
			GetViewPortInterface()->ShowPanel( PANEL_BUY_TER, false );
			GetViewPortInterface()->ShowPanel( PANEL_BUY_EQUIP_CT, false );
			GetViewPortInterface()->ShowPanel( PANEL_BUY_EQUIP_TER, false );
		}
	}
	...
	int x, int y, int width, int height )
{
	...
	if ( recreatePlayer )
	{
		...
		pPlayerModel->InitializeAsClientEntity( pModelName, RENDER_GROUP_OPAQUE );
	...
	if ( recreatePlayer || ShouldRecreateClassImageEntity( pWeaponModel, pWeaponName ) )
	{
		...
		pWeaponModel->InitializeAsClientEntity( pWeaponName, RENDER_GROUP_OPAQUE );
	...
	RenderableInstance_t instancea;

	pPlayerModel->DrawModel( STUDIO_RENDER, instancea);

	if ( pWeaponModel )
	{
		RenderableInstance_t instanceb;
		pWeaponModel->DrawModel( STUDIO_RENDER, instanceb);
	}
