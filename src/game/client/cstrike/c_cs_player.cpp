...
bool C_CSRagdoll::GetRagdollInitBoneArrays( matrix3x4_t *pDeltaBones0, matrix3x4_t *pDeltaBones1, matrix3x4_t *pCurrentBones, float boneDt )
{
	...
	ForceSetupBonesAtTime((matrix3x4a_t *)pDeltaBones0, gpGlobals->curtime - boneDt );
	GetRagdollCurSequenceWithDeathPose( this, (matrix3x4a_t *)pDeltaBones1, gpGlobals->curtime, m_iDeathPose, m_iDeathFrame );
	return SetupBones((matrix3x4a_t *)pCurrentBones, MAXSTUDIOBONES, BONE_USED_BY_ANYTHING, gpGlobals->curtime );
}
...
void C_CSRagdoll::CreateCSRagdoll()
{
	...
	if ( pPlayer && !pPlayer->IsDormant() )
	{
		...
		if ( bRemotePlayer )
		{
			...
			SetAbsAngles( pPlayer->GetRenderAngles() );
			GetRotationInterpolator().Reset(0);
...
bool C_CSRagdoll::IsTransparent( void )
{
	if ( m_flRagdollSinkStart == -1 )
	{
		return BaseClass::GetRenderAlpha() != 255;
	}
	else
	{
		return true;
...
void InitializeAddonModelFromWeapon( CWeaponCSBase *weapon, C_BreakableProp *addon )
{
	...
	{
		addon->InitializeAsClientEntity( weaponInfo.szWorldModel, RENDER_GROUP_OPAQUE );
	}
	else
	{
		addon->InitializeAsClientEntity( weaponInfo.m_szAddonModel, RENDER_GROUP_OPAQUE );
	}
}
...
void C_CSPlayer::CreateAddonModel( int i )
{
	...
	if ( addonType == ADDON_PISTOL || addonType == ADDON_PRIMARY )
	{
		...
		if ( weaponInfo->m_szAddonModel[0] == 0 )
		{
			pEnt->InitializeAsClientEntity( weaponInfo->szWorldModel, RENDER_GROUP_OPAQUE );
		}
		else
		{
			pEnt->InitializeAsClientEntity( weaponInfo->m_szAddonModel, RENDER_GROUP_OPAQUE );
		}
	}
	else if( pAddonInfo->m_pModelName )
	{
		...
		{
			pEnt->InitializeAsClientEntity( pAddonInfo->m_pHolsterName, RENDER_GROUP_OPAQUE );
		}
		else
		{
			pEnt->InitializeAsClientEntity( pAddonInfo->m_pModelName, RENDER_GROUP_OPAQUE );
		}
	}
	else
	{
		...
		{		
			if ( pWeaponInfo->m_szAddonModel[0] == 0 )
				pEnt->InitializeAsClientEntity( pWeaponInfo->szWorldModel, RENDER_GROUP_OPAQUE );
			else
				pEnt->InitializeAsClientEntity( pWeaponInfo->m_szAddonModel, RENDER_GROUP_OPAQUE );
		}
		else
		{
			pEnt->Release();
...
void C_CSPlayer::UpdateIDTarget()
{
	...
	VectorMA( MainViewOrigin(0), 2500, MainViewForward(0), vecEnd );
	...
	if ( !tr.startsolid && !tr.DidHitNonWorldEntity() )
	{
		...
		UTIL_ClipTraceToPlayers(vecStart, vecEnd + MainViewForward(0) * rayExtension, MASK_SOLID|CONTENTS_HITBOX, &filter, &tr );
	}
		...
		if ( pEntity && (pEntity != this) )
		{
			...
			for ( int i = 0; i < m_SmokeGrenades.Count(); i++ )
			{
				...
				if ( pSmokeGrenade )
				{
					...
					if ( IntersectInfiniteRayWithSphere( MainViewOrigin(0), MainViewForward(0), vPos, flRadius, &flHit1, &flHit2 ) )
					{
						 return;
					}
				}
...
void C_CSPlayer::BuildTransformations( CStudioHdr *pHdr, Vector *pos, Quaternion q[], const matrix3x4_t& cameraTransform, int boneMask, CBoneBitList &boneComputed )
{
	...
	if ( FindWeaponAttachmentBone( pWeapon, iWeaponBone ) )
	{
		...
		if ( FindMyAttachmentBone( this, iMyBone, pHdr ) )
		{
			...
			Studio_SolveIK( iShoulder, iElbow, iHand, vHandTarget, (matrix3x4a_t *)pBones );
...
void C_CSPlayer::PlayReloadEffect()
{
	...
	for ( int iSeq=0; iSeq < studioHdr.GetNumSeq(); iSeq++ )
	{
		...
		if ( pSeq->activity == ACT_VM_RELOAD )
		{
			...
			{
				mstudioevent_t *pEvent = (mstudioevent_t *)pSeq->pEvent( iEvent );
