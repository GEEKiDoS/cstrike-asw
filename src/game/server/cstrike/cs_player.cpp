...
BuyResult_e CCSPlayer::BuyGunAmmo( CBaseCombatWeapon *pWeapon, bool bBlinkMoney )
{
	...
	if ( GetAmmoCount( nAmmo ) >= GetAmmoDef()->MaxCarry( nAmmo,this ) )
	{
		return BUY_ALREADY_HAVE;
	}
...
void CCSPlayer::State_Enter_OBSERVER_MODE()
{
	...
	else
	{
		...
		if ( IsNetClient() )
		{
			const char *pIdealMode = engine->GetClientConVarValue(IndexOfEdict(edict()), "cl_spec_mode" );
			if ( pIdealMode )
			{
	...
	*/
	...
	if ( bPickupGrenade )
	{
		...
		if( pOwnedGrenade )
		{
			...
			maxGrenades = GetAmmoDef()->MaxCarry(ammoIndex,this);
...
void CCSPlayer::AutoBuy()
{
	...
	const char *autobuyString = engine->GetClientConVarValue(IndexOfEdict(edict()), "cl_autobuy" );
...
void CCSPlayer::Rebuy( void )
{
	...
	const char *rebuyString = engine->GetClientConVarValue(IndexOfEdict(edict()), "cl_rebuy" );
