...
bool CCSGameRules::FShouldSwitchWeapon(CBasePlayer *pPlayer, CBaseCombatWeapon *pWeapon)
{
	...
	if (pPlayer->GetActiveWeapon() && pPlayer->IsNetClient() && !bIsBeingGivenItem)
	{
		...
		const char *cl_autowepswitch = engine->GetClientConVarValue(IndexOfEdict(pPlayer->edict()), "cl_autowepswitch");
	...
	public:
		...
		virtual CBaseEntity* CreateNextEntity(const char *pClassname)
		{
			...
			else
			{
				...
				if (ref.m_iEdict == -1 || PEntityOfEntIndex(ref.m_iEdict))
				{
...
void CCSGameRules::ClientSettingsChanged(CBasePlayer *pPlayer)
{
	...
	{
		const char *pShowHints = engine->GetClientConVarValue(IndexOfEdict(pCSPlayer->edict()), "cl_autohelp");
